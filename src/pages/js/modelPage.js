...
export default function ModelPage() {
  ...
  return (
    <div className="modelPage">
      <h1 className="modelTitle animate__animated animate__fadeInLeft">
        Build New Model
      </h1>
      <button
        disabled={state === "loading"}
        onClick={handleTrain}
        className="train animate__animated animate__fadeInRightBig"
      >
        Train Model
      </button>
      <div className="modelCont row">
        ...
        <div className="col-lg-6 animate__animated animate__fadeInDown">
          <div className="modelResults">
            <h1>Settings</h1>
            {model.Model === "SVM" ? (
              <SvmParams setModel={setModel} model={model} />
            ) : model.Model === "Random Forest" ? (
              <RandomForestParams setModel={setModel} model={model} />
            ) : model.Model === "KNN" ? (
              <KNNParams setModel={setModel} model={model} />
            ) : model.Model === "Mean Shift" ? (
              <MeanShiftParams setModel={setModel} model={model} />
            ) : null}
          </div>
        </div>
      </div>
      <div className="modelCont row">
        <div className="col-lg-3 animate__animated animate__fadeInLeftBig">
          <div className="modelResults results">
            <h1>Precision</h1>
            <div className="resBody">
              {state === "none" ? (
                <p>Click on Train Model to view the Precision.</p>
              ) : state === "loading" ? (
                <Loading />
              ) : state === "done" ? (
                <p className="result">{results.precision}</p>
              ) : null}
            </div>
          </div>
        </div>
        <div className="col-lg-3 animate__animated animate__fadeInUp">
          <div className="modelResults results">
            ...
            <div className="resBody">
              {state === "none" ? (
                <p>Click on Train Model to view the Accuracy.</p>
              ) : state === "loading" ? (
                <Loading />
              ) : state === "done" ? (
                <p className="result">{results.accuracy} </p>
              ) : null}
            </div>
          </div>
        </div>
        <div className="col-lg-3 animate__animated animate__fadeInUpBig">
          <div className="modelResults results">
            <h1>F1 Score</h1>
            <div className="resBody">
              {state === "none" ? (
                <p>Click on Train Model to view the F1 Score.</p>
              ) : state === "loading" ? (
                <Loading />
              ) : state === "done" ? (
                <p className="result">{results.f1_score} </p>
              ) : null}
            </div>
          </div>
        </div>
        <div className="col-lg-3 animate__animated animate__fadeInRight">
          <div className="modelResults results">
            <h1>Confusion Matrix</h1>
            <div className="resBody">
              {state === "none" ? (
                <p>Click on Train Model to view the Confusion Matrix.</p>
              ) : state === "loading" ? (
                <Loading />
              ) : state === "done" ? (
                <div>
                  <p className="result">
                    TP:{results.confusion_matrix[0][0]} FP:
                    {results.confusion_matrix[0][1]}
                  </p>
                  <p className="result">
                    FN:{results.confusion_matrix[1][0]} TN:
